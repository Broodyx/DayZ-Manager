...
class MapEditor extends Page
{
    ...
    public function loadPointTypeCatalog(): void
    {
        ...
        $eventFields = [
            ...
            ['name' => 'event_active', 'edit_only' => true, 'section' => 'Nastavení eventu · events.xml', 'label' => 'Zapnuto (event skutečně spawnuje)', 'type' => 'checkbox', 'default' => true, 'help' => 'Odškrtni a event zůstane definovaný v events.xml, ale server ho přestane používat — nic už nebude spawnovat.'],
            ['name' => 'event_deletable', 'edit_only' => true, 'section' => 'Nastavení eventu · events.xml', 'label' => 'Lze sebrat/zničit (deletable)', 'type' => 'checkbox', 'default' => false, 'help' => 'Zaškrtnuto = instanci jde odstranit (typicky u loot eventů). Nezaškrtnuto = zmizí jen despawnem přes lifetime.'],
            ['name' => 'event_init_random', 'edit_only' => true, 'section' => 'Nastavení eventu · events.xml', 'label' => 'Náhodný start (init_random)', 'type' => 'checkbox', 'default' => false, 'help' => 'Zaškrtnuto = při startu serveru se lifetime první instance náhodně zkrátí, aby se spawny časem rozprostřely místo nástupu najednou.'],
            ['name' => 'event_remove_damaged', 'edit_only' => true, 'section' => 'Nastavení eventu · events.xml', 'label' => 'Odstranit poškozené (remove_damaged)', 'type' => 'checkbox', 'default' => false, 'help' => 'Zaškrtnuto = poškozená instance (např. mrtvé zvíře) se rovnou odstraní, místo aby čekala na despawn přes lifetime.'],
        ...
        foreach ($revisions as $revision) {
            ...
            foreach ($parsedMarkers as $marker) {
                ...
                if ($filename === 'cfgeventspawns.xml') {
                    ...
                    $marker['spawn_classnames'] = $children;
