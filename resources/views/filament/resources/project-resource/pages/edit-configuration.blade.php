    ...
    <div class="space-y-4">
        ...
        @if ($mode === 'visual' && $visualSupported)
            ...
            @elseif ($visualKind === 'server')
                @php
                $cfgDescriptions = ['hostname'=>'Název serveru zobrazovaný v prohlížeči serverů.','maxPlayers'=>'Maximální počet současně připojených hráčů.','password'=>'Heslo potřebné pro připojení na server.','passwordAdmin'=>'Heslo pro administrátorské příkazy ve hře.','enableWhitelist'=>'Zapíná kontrolu UID v whitelist.txt.','verifySignatures'=>'Ověřuje podpisy modifikací při připojení.','forceSameBuild'=>'Povolí připojení pouze se stejným buildem hry.','disableVoN'=>'Vypíná vestavěnou hlasovou komunikaci.','vonCodecQuality'=>'Kvalita kodeku hlasové komunikace (0–10).','disable3rdPerson'=>'Zakáže pohled ze třetí osoby.','disableCrosshair'=>'Zakáže zaměřovací kříž.','disablePersonalLight'=>'Zakáže osobní světlo hráče.','lightingConfig'=>'Profil osvětlení a barevného podání světa.','serverTime'=>'Čas, kterým server začne po restartu.','serverTimeAcceleration'=>'Násobek rychlosti plynutí denního času.','serverNightTimeAcceleration'=>'Násobek rychlosti plynutí nočního času.','serverTimePersistent'=>'Zachová herní čas mezi restarty serveru.','guaranteedUpdates'=>'Režim garantovaných síťových aktualizací.','loginQueueConcurrentPlayers'=>'Počet hráčů zpracovávaných současně ve frontě.','loginQueueMaxPlayers'=>'Maximální počet čekajících hráčů ve frontě.','instanceId'=>'Číselná identifikace instance serveru pro persistence.','storeHouseStateDisabled'=>'Vypíná ukládání stavu předmětů v kontejnerech.','storageAutoFix'=>'Automaticky opravuje poškozené persistence soubory.','template'=>'Název mapového economy profilu.','steamPort'=>'UDP port používaný pro Steam komunikaci.','steamQueryPort'=>'Port pro dotazy na server ve Steamu.','disableVoN'=>'Vypíná hlasovou komunikaci ve hře.','networkRange'=>'Dosah síťové synchronizace objektů.','defaultCharacter'=>'Výchozí typ postavy nového hráče.'];
                @endphp
                <section class="dz-editor-card"><h3>Serverová nastavení · serverDZ.cfg</h3><p class="dz-muted">Upravujte hodnoty serveru bez ručního psaní CFG syntaxe. Hesla jsou skrytá; prázdné pole zachová původní hodnotu.</p><div class="dz-fields-grid">
                    @foreach ($serverConfig as $key => $value)
                        ...
                        @endphp
                        <label class="dz-field"><span>{{ $key }}</span><input type="{{ $secret ? 'password' : 'text' }}" autocomplete="{{ $secret ? 'new-password' : 'off' }}" placeholder="{{ $secret ? 'Ponechte prázdné pro zachování' : '' }}" wire:model.defer="serverConfig.{{ $key }}"><small>{{ $cfgDescriptions[$key] ?? 'Nastavení „'.$key.'“ ovlivňuje chování serveru podle pravidel DayZ.' }}</small><small>Raw: {{ $key }} = {{ $secret ? '•••••••• (skryto)' : $value }};</small></label>
                    @endforeach
